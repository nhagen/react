...
You can also provide an object to `transitionName` which specifies custom class names, enabling you to use outside CSS libraries for transitions. You can modify the above example to use a library with pre-defined bounce in and bounce out classes:

```javascript
var TodoList = ReactClass({
  ...
  render: function() {
